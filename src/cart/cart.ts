...
export class CartOps {
    ...
    private readonly itemFolding: boolean;
    private nextUID: UID = 0;

    constructor(catalog: Catalog, showPIDs = false, itemFolding = true) {
        this.catalog = catalog;
        this.showPIDs = showPIDs;
        this.itemFolding = itemFolding;
    }
    ...
    private updateItem(item: ItemInstance, pid: PID, quantity: number): ItemInstance {
        if (this.itemFolding && pid === item.pid) {
